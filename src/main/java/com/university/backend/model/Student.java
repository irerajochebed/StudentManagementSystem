    ...
    public void enrollInCourse(Course course) {
        ...
        for (int i = 0; i < enrollments.size(); i++) {
            ...
            if (e.getCourse().getCourseCode().equals(course.getCourseCode())) {
                throw new com.university.backend.exception.DuplicateEnrollmentException(getFullName(), course.getCourseName());
            }
        }

        // Check if course is full
        if (course.isFull()) {
            throw new com.university.backend.exception.CourseFullException(course.getCourseName(), course.getMaxCapacity());
        }
